...
type CKYTable = Map.Map (Int, Int) [(LexicalEntry, String)]

-- A functional version of the CKY parser
-- TODO: Finish parsing spans of length > 1
generateFunctional :: String -> CKYTable
generateFunctional utterance = ckyTable
  where
...
emptyCKYEntry :: [(LexicalEntry, String)]
emptyCKYEntry = []
...
generate utterance = Map.fromList $ filter (not . null . snd) (assocs arr)
  where
    tokens = words utterance
    n = length tokens
    enumeratedTokens = zip [1..] tokens
    ...
    arr = runSTArray $ do
      marr <- newArray ((0, 0), (n, n)) emptyCKYEntry
      forM_ (enumeratedTokens) $ \(idx, token) ->
        writeArray marr (idx - 1, idx) (filterLexicon token)
      return marr
...
main = do
    ...
    putStrLn $ show $ generateFunctional "one plus one"
